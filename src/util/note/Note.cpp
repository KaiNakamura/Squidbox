...
Note getPreviousNote(Note note, Note minNote, Note maxNote, bool wrap) {
  return (note == minNote) ? (wrap ? maxNote : minNote)
                           : static_cast<Note>(note - 1);
}
...
Note getNextNote(Note note) { return getNextNote(note, false); }

Note getPreviousNote(Note note) { return getPreviousNote(note, false); }
